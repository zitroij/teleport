...
func (s *ServerSuite) TestStartStop(c *check.C) {
	called := false
	fn := NewChanHandlerFunc(func(_ *ConnectionContext, _ net.Conn, conn *ssh.ServerConn, nch ssh.NewChannel) {
		called = true
...
func (s *ServerSuite) TestHostSignerFIPS(c *check.C) {
	...
	newChanHandler := NewChanHandlerFunc(func(_ *ConnectionContext, _ net.Conn, conn *ssh.ServerConn, nch ssh.NewChannel) {
		nch.Reject(ssh.Prohibited, "nothing to see here")
	})
